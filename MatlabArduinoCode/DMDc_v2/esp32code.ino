    ...
    if (micros() - currentTimeMicros > 20000) {
        ...
        switch (state) {
            ...
            case 3:
                ...
                LqrCommand = accInMetersPerSecondSquared;
        ...
        if (printFlag) {
            data.position = getPosInCm();
            data.velocity = getCartVelocity();
            data.angle = getAngleInRad();
            data.angularVelocity = getAngularVelocity();
            data.command = LqrCommand;
            data.chirpSignal = systemIdentificationMode ? generateChirpSignal() : 0.0;
            data.potmet = readPotentiometer();
            // printAllStates();
            printAllStatesStruct();
        }
    }
}
