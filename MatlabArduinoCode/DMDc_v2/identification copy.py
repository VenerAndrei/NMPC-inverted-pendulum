...
# --- After computing A (closed-loop) and B via DMDc ---
K = np.array([-12.1732, -11.3985, -59.1152, -13.8610])  # LQR gain

# Compute open-loop A
A_open = A - B @ K.reshape(1, -1)  # Now B (4,1) @ K (1,4) -> (4,4)

print("Open-loop A matrix:")
print(A_open)
print("Closed-loop A matrix (from DMDc):")
print(A)
print("B matrix (from DMDc):")
print(B)

# Compute eigenvalues (poles) of the open-loop and closed-loop systems
eig_open = np.linalg.eig(A_open)[0]
eig_closed = np.linalg.eig(A)[0]

print("\nOpen-loop poles (eigenvalues of A_open):")
print(eig_open)

print("\nClosed-loop poles (eigenvalues of A_closed):")
print(eig_closed)

# Optional: Plot poles in the complex plane
plt.figure()
plt.scatter(np.real(eig_open), np.imag(eig_open), marker='x', color='r', label='Open-loop poles')
plt.scatter(np.real(eig_closed), np.imag(eig_closed), marker='o', color='b', label='Closed-loop poles')
