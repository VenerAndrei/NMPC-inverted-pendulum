...
# Plotting each state in a 2 by 2 grid
fig, axs = plt.subplots(2, 2, figsize=(12, 8), sharex=True)
states = ["Cart Position (x)", "Cart Velocity (dx)", "Pendulum Angle (th)", "Pendulum Angular Velocity (dth)"]
y_labels = ["Position (m)", "Velocity (m/s)", "Angle (rad)", "Angular Velocity (rad/s)"]

for i in range(4):
    ax = axs[i // 2, i % 2]
    ax.plot(time_steps, X_log[i, :], label=states[i], linewidth=1.5)
    ax.grid(True)
    ax.set_ylabel(y_labels[i])
    ax.legend(loc='upper right', fontsize='small')

axs[0, 0].set_title('States vs. Time')
plt.xlabel('Time (seconds)')
plt.tight_layout()
plt.savefig("CartPoleStates-{}.png".format(timestr), dpi=300)

# Plotting the input command U in a separate figure
plt.figure(figsize=(10, 4))
plt.plot(time_steps, U_log.T, label='Input Command U', color='tab:purple', linewidth=1.5)
plt.axhline(y=10, color='r', linestyle='--', label='Upper Limit (10)')
plt.axhline(y=-10, color='r', linestyle='--', label='Lower Limit (-10)')
plt.grid(True)
plt.title('Input Command vs. Time')
plt.xlabel('Time (seconds)')
plt.ylabel('Force (N)')
plt.legend()
plt.tight_layout()
plt.savefig("CartPoleInputCommand-{}.png".format(timestr), dpi=300)
plt.show()
